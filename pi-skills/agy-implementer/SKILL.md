...
- `--continue` (or `--conversation <id>`), `--model <id>` / `--effort low|medium|high` (default `gemini-3.8-flash-high`; when to override: `../agy-jobs/references/model-routing.md`), `--timeout <dur>` (default 60m, maximum 120m hard execution limit).
