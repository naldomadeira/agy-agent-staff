...
- `--model <id>` / `--effort low|medium|high` (default `gemini-3.8-flash-medium`; when to override: `../agy-jobs/references/model-routing.md`), `--continue` (or `--conversation <id>`), `--timeout <dur>` (default 60m, maximum 120m hard execution limit).
